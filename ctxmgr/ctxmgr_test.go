...
func TestSimpleCounter_Basic(t *testing.T) {
	...
	en, err := c.Count(ctx, CountRequest{Messages: []core.Message{{Role: core.RoleUser, Content: strings.Repeat("a", 400)}}})
	if err != nil {
		t.Fatal(err)
	}
	zh, err := c.Count(ctx, CountRequest{Messages: []core.Message{{Role: core.RoleUser, Content: strings.Repeat("中", 300)}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSimpleCounter_IncludesTools(t *testing.T) {
	...
	without, err := c.Count(ctx, CountRequest{Messages: []core.Message{{Content: "hi"}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTruncateStrategy_KeepsRecent(t *testing.T) {
	...
	for i := range msgs {
		msgs[i] = core.Message{Role: core.RoleUser, Content: "m"}
	}
...
func TestTruncateStrategy_NoOpWhenShort(t *testing.T) {
	msgs := []core.Message{{Role: core.RoleUser, Content: "a"}}
	s := &TruncateStrategy{KeepRecent: 10}
	out, action, err := s.Apply(context.Background(), msgs, 100)
	if err != nil {
		t.Fatal(err)
...
func TestTruncateStrategy_PreservesToolPairing(t *testing.T) {
	msgs := []core.Message{
		...
		{Role: core.RoleTool, ToolCallID: "c1", Content: "r1"},
		{Role: core.RoleTool, ToolCallID: "c2", Content: "r2"},
		{Role: core.RoleTool, ToolCallID: "c3", Content: "r3"},
		{Role: core.RoleAssistant, Content: "final"},
	}

	// KeepRecent=4 → 候选起点 index 4（core.RoleTool）→ 必须前移到 index 3。
	...
	if out[0].Role == core.RoleTool {
		t.Fatalf("cut left a dangling tool result: %+v", out[0])
	}
	if out[0].Role != core.RoleAssistant || len(out[0].ToolCalls) != 3 {
...
func TestOffloadStrategy(t *testing.T) {
	...
	msgs := []core.Message{
		...
		{Role: core.RoleTool, ToolCallID: "c2", Content: "small"},
	}
...
func TestOffloadStrategy_NoOffloader(t *testing.T) {
	...
	out, action, err := s.Apply(context.Background(), []core.Message{{Role: core.RoleTool, Content: "x"}}, 1)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMemoryOffloader_NotFound(t *testing.T) {
	off := NewMemoryOffloader()
	if _, err := off.Retrieve(context.Background(), "nope"); err != core.ErrNotFound {
...
func TestDefaultContextManager_AppliesStrategies(t *testing.T) {
	m := NewDefaultContextManager(NewSimpleCounter())
	msgs := make([]core.Message, 200)
	for i := range msgs {
		msgs[i] = core.Message{Role: core.RoleUser, Content: strings.Repeat("x", 200)}
	}
