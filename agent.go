...
func New(cfg Config) (*Agent, error) {
	if cfg.Model == nil {
		return nil, newError(ErrKindConfig, "agent", "new", errors.New("Model is required"))
	}
	...
	ctxMgr := cfg.Context
	if ctxMgr == nil {
		ctxMgr = NewDefaultContextManager(counter)
	}
...
//
// 若 Config.Runner 被替换，本方法不生效，工具应通过 RunnerOptions 提供。
func (a *Agent) RegisterTool(t Tool) error {
	r, ok := a.runner.(*ReActRunner)
	if !ok {
		return newError(ErrKindConfig, "agent", "register tool",
...
func (a *Agent) Run(ctx context.Context, sessionID, input string) iter.Seq2[Event, error] {
