...
func (s *TruncateStrategy) Apply(ctx context.Context, msgs []Message, budget int) ([]Message, *ContextAction, error) {
...
func safeCutPoint(msgs []Message, start int) int {
	for start > 0 && msgs[start].Role == RoleTool {
		start--
	}
	return start
}
...
// Apply 实现 ContextStrategy。
func (s *OffloadStrategy) Apply(ctx context.Context, msgs []Message, budget int) ([]Message, *ContextAction, error) {
	if s.Offloader == nil {
		return msgs, nil, nil
	}
	if err := ctx.Err(); err != nil {
	...
	offloaded := 0
	for i := range out {
		if out[i].Role != RoleTool || len(out[i].Content) <= threshold {
			continue
		}
