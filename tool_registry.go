...
func (r *ToolRegistry) Register(t Tool) error {
	if t == nil {
		return newError(ErrKindTool, "", "register", fmt.Errorf("tool is nil"))
	}
	info := t.Info()
	if info.Name == "" {
		return newError(ErrKindTool, "", "register", fmt.Errorf("tool name is empty"))
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	if _, exists := r.tools[info.Name]; exists {
		return newError(ErrKindTool, info.Name, "register", fmt.Errorf("duplicate tool name"))
	}
	r.tools[info.Name] = t
	return nil
}
