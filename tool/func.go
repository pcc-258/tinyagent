...
type FuncTool[In any] struct {
	info ToolInfo
	fn   func(context.Context, In) (core.ToolResult, error)
}
...
//
// 注意：返回的 core.ToolResult 无需填写 ToolCallID，运行循环会补上。
func NewFuncTool[In any](
	name, desc string,
	fn func(context.Context, In) (core.ToolResult, error),
) (*FuncTool[In], error) {
	if name == "" {
		return nil, core.NewError(core.ErrKindTool, "", "new", fmt.Errorf("tool name is empty"))
	}
	if fn == nil {
		return nil, core.NewError(core.ErrKindTool, name, "new", fmt.Errorf("tool function is nil"))
	}

	schema, err := schemaFromType(reflect.TypeOf((*In)(nil)).Elem())
	if err != nil {
		return nil, core.NewError(core.ErrKindTool, name, "schema", err)
	}
...
// Run 实现 Tool。
//
// 参数解析失败时返回 core.ToolResult{Error: ...}（业务软错误）而非 error ——
// 这通常意味着模型给错了参数，应该把原因喂回去让它自行纠正。
func (t *FuncTool[In]) Run(ctx context.Context, args json.RawMessage) (core.ToolResult, error) {
	var in In
	if len(args) > 0 {
