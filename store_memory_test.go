...
func TestMemoryStore_AppendAndLoad(t *testing.T) {
	...
	if err := st.Append(ctx, "s1", Message{Role: RoleUser, Content: "hi"}); err != nil {
		t.Fatal(err)
	}
	if err := st.Append(ctx, "s1", Message{Role: RoleAssistant, Content: "hello"}); err != nil {
		t.Fatal(err)
	}
...
func TestMemoryStore_Replace(t *testing.T) {
	...
	if err := st.Append(ctx, "s1",
		Message{Content: "a"}, Message{Content: "b"}, Message{Content: "c"}); err != nil {
		t.Fatal(err)
	}

	if err := st.Replace(ctx, "s1", []Message{{Content: "summary"}}); err != nil {
		t.Fatal(err)
	}
	snap, err := st.Load(ctx, "s1")
	if err != nil {
...
func TestMemoryStore_SnapshotIsolation(t *testing.T) {
	st := NewMemoryStore()
	ctx := context.Background()
	if err := st.Append(ctx, "s1", Message{
		Role:      RoleAssistant,
		ToolCalls: []ToolCall{{ID: "c1", Name: "t", Arguments: []byte(`{"a":1}`)}},
...
func TestMemoryStore_ListAndDelete(t *testing.T) {
	st := NewMemoryStore()
	ctx := context.Background()
	if err := st.Append(ctx, "a", Message{Content: "1"}); err != nil {
		t.Fatal(err)
	}
	if err := st.Append(ctx, "b", Message{Content: "2"}); err != nil {
		t.Fatal(err)
	}
	...
	if err := st.Delete(ctx, "a"); !errors.Is(err, ErrNotFound) {
...
func TestMemoryStore_Concurrent(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			for j := 0; j < 50; j++ {
				_ = st.Append(ctx, "shared", Message{Content: "x"})
...
func TestSession_SnapshotIsolation(t *testing.T) {
	s := NewSession("s1")
	s.Append(Message{Role: RoleUser, Content: "hi", Meta: map[string]any{"k": "v"}})
